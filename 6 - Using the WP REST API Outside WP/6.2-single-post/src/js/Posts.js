...
export default class Posts {
    ...
    static loadSinglePost() {
        ...
        axios.get( config.rest_url + 'wp/v2/posts/' )
            .then( response => {
                ...
                Posts.renderPost( POST, IS_SINGLE );
            } );
    ...
    static renderPost( post, single = false ) {
        ...
        if( IS_SINGLE ) {
            ...
            article.querySelector( 'a.back-to-posts' ).addEventListener( 'click', LOAD_POSTS, false );
        } else {
            // Change LOAD_SINGLE_POST to Posts.loadSinglePost
            article.querySelector( '.entry-title a' ).addEventListener( 'click', LOAD_SINGLE_POST, false );
        }
